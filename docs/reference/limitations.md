...
A file opened with [`File::open_rw_bounded`](../guide/editing.md#bounded-memory-appends) offers the same edit surface as `File::open_rw` — reads, immediate `Dataset::append`, and the staged surface (`write`, attribute edits, `create_*`/`delete`, `copy`, `commit`, `space_accounting`) — with a commit that costs what the edit costs rather than what the file costs. It requires a latest-format file with 8-byte offsets and no userblock; reads have the [streaming backend's capabilities](../guide/streaming.md). It grows a file that persists its free space, including a genuine paged file (`H5F_FSPACE_STRATEGY_PAGE`); a paged file that does *not* persist its free space is refused with `Error::EditUnsupported` (recreate it with `persist = true`), by both editors.
