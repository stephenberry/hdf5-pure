...
A compact attribute is stored as an object-header message, and that message's size field is 2 bytes wide, so an attribute whose serialized message exceeds 65,535 bytes is refused with `FormatError::AttributeMessageTooLarge` naming it, rather than written with a truncated length. The limit is on the *message* — name, datatype, dataspace, and data — not on the element count, and it applies to root, group, and dataset attributes alike. Attributes are metadata; store a payload that large as a dataset instead.
