...
| A `maxshape` whose chunk index would leave more than 4,194,304 elements unused. The index is emitted in one pass, so unused elements are written to the file; they come from a `maxshape` wider than the shape in a dimension other than the one the dataset grows along, and chunking those dimensions more coarsely removes them. The reference library grows its index a block at a time and so accepts geometries this refuses | [#299](https://github.com/stephenberry/hdf5-pure/issues/299) |
