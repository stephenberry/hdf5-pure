...
| Adding or removing a **link** in a group that tracks **link creation order**, as netCDF-4 writes. Such a group is read, its attributes are edited, and a link already in it is retargeted when its child relocates; only changing which links it holds is refused, since a Link message this crate emits carries no creation index | [#416](https://github.com/stephenberry/hdf5-pure/issues/416) |
