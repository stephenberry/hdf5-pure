...
| Adding a **link** to a group that tracks **link creation order**, as netCDF-4 writes, since a Link message this crate emits carries no creation index. Such a group is otherwise editable: its attributes are set, a link is removed from it (which leaves a gap in the order and does not lower the group's creation-index counter), and a link already in it is retargeted when its child relocates | [#416](https://github.com/stephenberry/hdf5-pure/issues/416) |
