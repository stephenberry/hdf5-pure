...
A file opened with [`File::open_rw_bounded`](../guide/editing.md#bounded-memory-appends) supports reads and immediate `Dataset::append` only; the staged edit surface (`write`, attribute edits, `create_*`/`delete`, `copy`, `commit`, `space_accounting`) needs the whole-file mirror and returns `Error::BoundedStagedUnsupported` — open with `File::open_rw` for those. It requires a latest-format file with 8-byte offsets and no userblock; reads have the [streaming backend's capabilities](../guide/streaming.md). It grows a file that persists its free space, including a genuine paged file (`H5F_FSPACE_STRATEGY_PAGE`); a paged file that does *not* persist its free space is refused with `Error::EditUnsupported` (recreate it with `persist = true`), by both editors.
