...
| no C counterpart | `FileAccessProperties::with_memory_strategy`, `MemoryStrategy` | **Genuine** | How much memory a read-write open may spend holding the file, overriding the dispatch above. `Bounded` refuses a file the bounded engine cannot edit — a pre-v2 superblock or a userblock — rather than mirroring it; `Auto` (what `open_rw` uses unset) falls back to the mirror instead; `Mirrored` always mirrors. A paged file without persisted free space is refused under both preferences, because the mirror cannot commit it either. `File::memory_strategy` reports what an open resolved to. The C library has no analogue: `H5Fopen` picks its own caching with no caller-visible memory contract. |
