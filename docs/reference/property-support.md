...
| `H5Pset_libver_bounds` | `FileCreateProperties::with_libver_bounds`, `FileBuilder::with_libver_bounds` | **Assertion** | The writer always emits the v3 (HDF5 1.10) superblock; the bound is an accept/reject check, not a format selector. HDF5 classes this as an `fapl` property; this crate applies it at **write** time, so it sits on `FileCreateProperties` with the other write-time settings. |
