...
| `H5Pset_page_buffer_size` | none | **Not modeled** | A write-back page buffer holds dirty pages across the ordering barriers a commit or append defines, which reorders publish points ahead of the content they name. An implementation was written and measured, then held back on pairing it with an on-disk mark that makes a crashed session's file refuse to open ([#308](https://github.com/stephenberry/hdf5-pure/issues/308)). Every read-write session still gathers its writes and issues one per dirty page, releasing at every barrier; that needs no property and gives up nothing. |
