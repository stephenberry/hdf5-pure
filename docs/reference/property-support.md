...
| no C counterpart | `FileAccessProperties::with_sync_policy`, `SyncPolicy`, `File::sync` | **Behavioral** | Who issues the `fsync`s. The default `Always` forces durability at every point the write paths define one; `OnClose` issues none during the session — the cadence is the application's, through `File::sync` — and one at `close` or drop, which write past the point any caller could order them. Writes still reach the operating system by the time the operation making them returns either way, so what moves to the caller is power-loss durability within the session, not same-machine visibility — see [Choosing the fsync cadence](../guide/editing.md#choosing-the-fsync-cadence) for the cost in full. The C library has no property for this because it never `fsync`s at all: the default `sec2` driver installs no flush callback, so `H5Fflush` drains libhdf5's caches with `write` and stops. |
| `H5Pset_page_buffer_size` | none | **Not modeled** | A write-back page buffer holds dirty pages across the ordering barriers a commit or append defines, which reorders publish points ahead of the content they name: a failure mid-flush can leave a dataset that reads clean and returns fill values or a deleted object's bytes. The C library's page buffer reorders the same way and makes no crash-consistency claim, but this crate does make one, so an implementation was written, measured and then held back on pairing it with an on-disk mark that makes such a file refuse to open ([#308](https://github.com/stephenberry/hdf5-pure/issues/308)). Every read-write session still gathers its writes and issues one per dirty page, releasing at every barrier; that needs no property and gives up nothing. |
