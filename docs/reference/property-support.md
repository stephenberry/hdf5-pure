...
| `H5Pset_page_buffer_size` | `FileAccessProperties::with_page_buffer_size` | **Genuine (paged files)** | A write-back page buffer for a read-write session: dirty pages live across the ordering barriers inside a commit or append, up to the byte budget, then flush whole. Requires a `Page` file, a budget of at least both its page size and the 1 MiB a session already gathers under, a version-3 superblock, and `SyncPolicy::OnClose`. All five are refused rather than ignored — where `H5Fopen` silently zeroes the budget on an unpaged file and rounds a sub-page budget up (only `H5Fcreate` refuses) — as are the SWMR writer and any `File::create_with_options` pair that could not be reopened. It reorders publish points ahead of the content they name, so the session raises superblock status-flag bit 0 (`H5F_SUPER_WRITE_ACCESS`) for its lifetime: a writer that dies mid-flush leaves a file this crate, `H5Fopen` and h5py all refuse, rather than one that reads clean and returns fill values or a deleted object's bytes. The C library's page buffer reorders the same way and ships no such mark. Only the budget is modeled, not `min_meta_perc` / `min_raw_perc`: the buffer does not evict, so it has no victim to choose. Off by default, which still leaves the write gathering every read-write session does. |
...
**Adaptive resize is the one substantive gap, and it is deliberate.** libhdf5 grows and shrinks its cache on an observed hit rate because picking a fixed size is hard and over-provisioning is expensive there. Neither holds here: since [#367](https://github.com/stephenberry/hdf5-pure/issues/367) the store is indexed rather than scanned, so a hit costs the same at 1,000 entries as at 65,000, and an over-generous budget costs memory and nothing else. The advice a resize algorithm would implement — set it generously — is one line, and `File::metadata_cache_stats` is what confirms it landed. Note the direction the algorithm runs: a low hit rate makes it hold *more*. Before #367 that was the wrong direction on this store.
