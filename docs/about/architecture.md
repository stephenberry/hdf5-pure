...
`File::open_rw_bounded` is the same engine minus the mirror: reads are positioned I/O through bounded caches (the read-write sibling of [`open_streaming`](../guide/streaming.md)), `Dataset::append` reads and patches only the metadata windows it touches with the same crash-atomicity, and `close()` rewrites the on-disk free-space managers of a file that persists them — including the per-page-type managers of a paged file, whose appends stay page-homogeneous. The staged edit surface works here too: it builds what the edit needs, not a copy of the file. What separates the two entry points is which files they accept — the bounded one requires a latest-format file with 8-byte offsets and no userblock — and the [capability matrix](../guide/editing.md#choosing-a-write-path) documents the rest against each file-space strategy.
