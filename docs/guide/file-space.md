...
A paged file is grown in place through either editor. [`File::open_rw_bounded`](editing.md#bounded-memory-appends) appends to it in bounded memory and rewrites the per-page-type managers at `File::close`; [`File::open_rw`](editing.md) commits the full staged edit surface to it, through a page-aware tail that rewrites the same managers. Both keep each page homogeneous — raw data and metadata never share a page — and page-align the end of allocation, so the reference C library reopens the result as a paged file. A paged file created **without** `persist = true` has no on-disk record of which pages hold metadata versus raw data, so neither editor can keep the two segregated and both refuse it (`Error::EditUnsupported`) before any byte changes — recreate it with `persist = true` if you need to append to it later.
