...
These methods cover *reading* strings. On the write side there are two facilities: `DatasetBuilder::with_vlen_strings` writes a contiguous variable-length UTF-8 string dataset, and the attribute variant `AttrValue::VarLenAsciiArray` writes a variable-length ASCII string array as an attribute (see [Groups & attributes](groups-attributes.md)). Both store their payloads in the file's global heap, split across as many collections as their element count needs, and a chunked, filtered, or resizable variable-length-string dataset is refused; see [Limitations](../reference/limitations.md#writing). Reading has no such restriction. For how string datatypes map between HDF5 and this crate, see [Data types](../reference/data-types.md).
