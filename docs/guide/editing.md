...
Two more rules follow from what `commit` itself does. A creation whose name **collides** with a link the commit does not remove is refused *where it is staged* rather than at the commit, because there would be no new object for the handle to address: `create_dataset("existing")` returns `Error::EditUnsupported` unless the same session also deletes that name. And deleting something the session staged and has not committed **withdraws** that staging — its attributes, appends and staged children go with it — since there is no link in the file to unlink; when the file does hold a link at the path, the plain deletion of it is what remains. A handle onto a withdrawn creation reports `Error::StagingWithdrawn` from then on: it names nothing, and the object the file holds at that path is the one the session is deleting.
