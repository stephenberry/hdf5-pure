...
Two more rules follow from what `commit` itself does. A creation whose name **collides** with a link the commit does not remove is one the commit refuses, so it does not shadow the object in the file: reads and listings go on answering with what is there. And deleting something the session staged and has not committed **withdraws** that staging — its attributes, appends and staged children go with it — since there is no link in the file to unlink; when the file does hold a link at the path, the plain deletion of it is what remains.
