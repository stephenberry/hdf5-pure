...
//! refused.
//!
//! Refused (named, never dropped silently): variable-length, time (its byte
//! order is not modelled), and reference datatypes (a reference's stored
//! absolute addresses would go stale on rewrite); virtual and external data
//! layouts; a lossy filter on the contiguous re-encode or sparse-chunked
//! fallback path; and any attribute whose datatype the reader cannot decode into
//! an [`AttrValue`] (e.g. an enumeration, compound, or boolean attribute). An
//! attribute that cannot be reproduced fails the repack by name rather than
