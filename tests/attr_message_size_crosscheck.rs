...
//! (issue #190).
//!
//! The writer now refuses an attribute whose object-header message would not fit
//! the header's 2-byte size field. That boundary is only worth anything if the
//! largest attribute it still accepts produces a file the reference C library
//! reads correctly — otherwise the limit would just be sitting in the wrong
...
fn largest_accepted_i64_attr() -> Vec<i64> {
    ...
    panic!("no i64 attribute size was accepted");
}

#[test]
fn c_reads_the_largest_accepted_root_attribute() {
    let values = largest_accepted_i64_attr();
