...
fn a_dense_object_takes_a_new_attribute() {
    ...
    assert_eq!(v, 99);
    let v: i64 = ds.attr("attr_00").unwrap().read_scalar().unwrap();
    assert_eq!(v, 0);
    let v: i64 = c
        .group("g")
        .unwrap()
...
fn a_dataset_added_in_place_may_carry_a_dense_attribute_set() {
    ...
    let c = hdf5::File::open(&p).unwrap();
    for name in ["contig", "chunked"] {
        let ds = c.dataset(name).unwrap();
        assert_eq!(ds.attr_names().unwrap().len(), DENSE_COUNT, "{name}");
        let v: i64 = ds.attr("attr_07").unwrap().read_scalar().unwrap();
        assert_eq!(v, 7, "{name}");
    }
}
