...
- `File::open_rw` opens a file for reading **and** writing through owned handles: `Dataset::append` grows a chunked, unlimited, Extensible-Array-indexed dataset in place (immediate and crash-atomic, reading back through the same handle), while `Dataset::write`/`set_attr`/`remove_attr` and `Group::create_group`/`delete` stage edits that `File::commit` applies as one transaction. A write on a read-only file returns `Error::ReadOnly` ([#148](https://github.com/stephenberry/hdf5-pure/issues/148)).
...
- `AppendWriter` appends to an existing **chunked, unlimited dataset** in place at amortized `O(1)` cost per append, held open across many appends rather than re-reading and rebuilding the index on every commit like `EditSession::append_dataset`. Unfiltered datasets accept **any-length** appends; **filtered** datasets must be appended in whole chunks, so a non-chunk-aligned filtered append is refused (use `EditSession::append_dataset` instead) ([#145](https://github.com/stephenberry/hdf5-pure/issues/145)).
