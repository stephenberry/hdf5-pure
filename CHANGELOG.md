...
- A refused `File::commit` puts back the values it had already written over, so a failed commit leaves every dataset reading what it read before rather than applying a same-length `Dataset::write` while discarding the rest of the batch. The new `Error::CommitPartiallyApplied` reports the one case where that restore itself failed ([#344](https://github.com/stephenberry/hdf5-pure/issues/344)).
