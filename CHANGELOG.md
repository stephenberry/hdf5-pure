...
- SWMR (single-writer / multiple-reader) support for one-dimensional, unlimited, Extensible-Array-indexed datasets ([#17](https://github.com/stephenberry/hdf5-pure/issues/17)).
  ...
  - Current subset: unfiltered datasets, chunk-aligned appends, a single unlimited dimension, and no userblock; filtered datasets return a clear error rather than producing an inconsistent file. SWMR read/append require the `std` filesystem and are unavailable on the in-memory/WASM path.
