...
- `Dataset::append` refuses to grow a partial trailing chunk under a **lossy** filter pipeline (ZFP, or float D-scale scale-offset), where re-encoding it would change values that are already committed; such a dataset takes whole-chunk appends from a chunk-aligned length ([#393](https://github.com/stephenberry/hdf5-pure/issues/393)).
- `Dataset::append_staged` refuses to grow a partial trailing chunk under a **lossy** filter pipeline (ZFP, or float D-scale scale-offset), matching `Dataset::append`; re-encoding it changed committed values, measured as `1.45` reading back as `1.5` at one decimal. Such a dataset takes whole-chunk appends from a chunk-aligned length ([#407](https://github.com/stephenberry/hdf5-pure/issues/407)).
