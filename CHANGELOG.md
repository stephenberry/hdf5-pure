...
- **Breaking:** `AttrValue`, `DType`, `Datatype`, `ReferenceType`, `LibVer`, `CompoundMember`, `EnumMember`, and `FileSpaceInfo` are now `#[non_exhaustive]`, so a new datatype, reference kind, or library-version bound no longer breaks callers. Add a `_` arm when matching a read-back value; constructing the existing variants is unaffected, including `Datatype` literals for types this crate has no constructor for. The enums whose variant set the format fixes (`FileSpaceStrategy`, `FileLocking`, `DatatypeByteOrder`, `StringPadding`, `CharacterSet`) stay exhaustive ([#206](https://github.com/stephenberry/hdf5-pure/pull/206)).
