...
- `MatError::from_source` carries a caller's own error type through the builder's closures and `DataProducer` as the new `MatError::Source`, so an embedding crate recovers it with `Error::source` and `downcast_ref` instead of flattening it to text through `MatError::Custom`.
