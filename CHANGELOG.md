...
- Reading persistent free-space managers (issue [#21](https://github.com/stephenberry/hdf5-pure/issues/21)). For a file written with `persist = true`, `File::persisted_free_space()` follows the File Space Info message's manager addresses to the on-disk free-space-manager blocks (the `FSHD` header and `FSSE` section list) and returns the tracked free `(address, length)` regions. The block parser is verified byte-for-byte against HDF5 1.14.6 and end-to-end against a file the reference C library writes with a deleted dataset. This is the read half of file-space persistence; writing the managers (so this crate's own edits persist across reopen) is the next step.
