...
- **Breaking:** An object reference supplied as an *address* is screened against the same commit's deletions the way one named as a path already was: `with_reference_data`, `with_raw_data`, and an in-file `File::copy` re-emitting a source's references all wrote them as-is, leaving a dereference that resolved to whatever the next commit put in that space. Copying a *chunked* object-reference dataset beside a delete is refused rather than screened, since its addresses sit inside chunks the copy path does not decode ([#317](https://github.com/stephenberry/hdf5-pure/issues/317)).
