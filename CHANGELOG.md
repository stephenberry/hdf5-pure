...
- **Breaking:** `File::group` and `Group::group` refuse a name that reaches something other than a group with the new `Error::NotAGroup`, the way `H5Gopen` does, instead of handing back a handle whose `attrs()` answered with the dataset's attributes and whose `datasets()` failed later naming no path. A commit that replaces a group with a dataset at the same path leaves a live handle reporting the same error ([#352](https://github.com/stephenberry/hdf5-pure/issues/352)).
