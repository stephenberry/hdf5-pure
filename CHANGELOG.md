...
- Repack now copies a chunked dataset's compressed chunks **verbatim** instead of decompressing and re-compressing them. Each chunk is read one at a time and written byte-for-byte with the source's filter-pipeline message carried through unchanged, so the repacked chunks are identical to the source's. This eliminates the per-dataset decompression blowup that made repack fail on large compressed files ([#82](https://github.com/stephenberry/hdf5-pure/issues/82)) and the decompress→recompress round-trip ([#84](https://github.com/stephenberry/hdf5-pure/issues/84)), and because nothing is ever decoded, **lossy** filters now survive byte-exact: float D-scale scale-offset, ZFP, SZIP, and even filters this crate cannot itself apply are all preserved ([#85](https://github.com/stephenberry/hdf5-pure/issues/85)). Verified by reading the repacked datasets back identically and by the reference C library decoding repacked deflate- and scale-offset-filtered datasets. The verbatim path applies to a dataset whose chunk grid is fully allocated (the common case); a *sparse* chunked dataset (with unallocated chunk-grid holes) still falls back to the read-raw + re-encode path, which requires a lossless filter, and a contiguous/compact filtered dataset likewise re-encodes — a lossy filter on either is still refused by name. What is removed is the per-dataset decompression and the lossy-filter loss; the remaining whole-file buffering is addressed below.
- Repack now **streams its output** instead of building the whole file in memory. The file is assembled directly onto the destination, and each chunked dataset's compressed chunks are pulled from the source and written one at a time rather than all collected first, so peak memory for the common chunked case drops from roughly 3× to about 1× of the (compressed) file size, regardless of dataset size. The streamed file is byte-for-byte identical to the buffered one. `FileBuilder::finish_to(writer)` streams any built file to a `std::io::Write` sink (now used by `FileBuilder::write`). The source is still read whole via `File::open`; streaming the source as well, for fully out-of-core repack, is the remaining half of [#82](https://github.com/stephenberry/hdf5-pure/issues/82), now completed below.
- Repack is now **fully out-of-core**, closing [#82](https://github.com/stephenberry/hdf5-pure/issues/82). It opens the source with `File::open_streaming`, so the source file is read on demand instead of buffered whole. Combined with the verbatim chunk copy and the streamed output above, peak memory across a repack is now bounded by one chunk plus the file's metadata, regardless of dataset (or file) size. Reaching this required closing the last two gaps in the streaming reader ([#27](https://github.com/stephenberry/hdf5-pure/issues/27)): the streaming backend now reads **attributes** (compact, shared, dense fractal-heap, and variable-length string) and traverses **v1 symbol-table groups** (local heap + version 1 B-tree + symbol-table nodes), reading each structure on demand. `File::attrs()` and group traversal on a streaming file now return the same results as the buffered backend, verified across compact/dense/VL and v1/v2 fixtures, and repack of an older-format (v1) source round-trips its groups and attributes.
...
- Object copy now reproduces dense (fractal-heap) attribute storage. Above the compact threshold of 8 attributes, HDF5 stores an object's attributes in a fractal heap indexed by a B-tree v2; `EditSession::copy` and `copy_from` previously refused any object whose Attribute Info message named a real heap. The copy now reads the source attributes out of that heap and re-emits them into a *fresh* single-direct-block fractal heap plus B-tree v2 name index in the destination — within one file and across two — so a dataset or group with many attributes copies and is read back, attribute names and values intact, by this crate's reader and the reference C library. The fresh heap is always destination-local (a same-file copy never aliases the source heap). The first cut tracks only the name index, not the creation-order index, and emits a single direct block: an attribute set too large for one direct block (one that would need fractal-heap indirect blocks) is refused by name rather than mis-encoded, and on the cross-file path a dense set whose values are variable-length or reference data is refused like its compact counterpart (its heap-resident bytes would embed source-file addresses) ([#87](https://github.com/stephenberry/hdf5-pure/issues/87)).
