...
- `FileAccessProperties::with_page_buffer_size` gives a read-write session a write-back page buffer, the `H5Pset_page_buffer_size` analogue: 32 chunk appends and a commit issued 5 writes where the default gathering issued 188. Such a session marks the file in the superblock for its lifetime, so one that dies mid-flush leaves a file this crate and the C library both refuse rather than one that reads clean, and it repays that mark's two `fsync`s only over a long session ([#308](https://github.com/stephenberry/hdf5-pure/issues/308)).
