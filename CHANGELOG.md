...
- A `maxshape` so much wider than the shape that its chunk index would need more than 4,194,304 elements is refused, naming an unlimited maximum as the alternative; the index is emitted in one pass, so its size follows the maximum shape rather than the data ([#299](https://github.com/stephenberry/hdf5-pure/issues/299)).
