...
- An object reference to a child of a group deleted in the same commit is refused rather than written pointing into the space that delete reclaims, where a later commit reusing the span left it dereferencing whatever landed there. The deleted path itself was already refused, and a path this commit deletes and puts back still resolves to the replacement ([#314](https://github.com/stephenberry/hdf5-pure/issues/314)).
