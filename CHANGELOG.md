...
- **Breaking:** the serde writer honors `Options::null_policy`, which it previously ignored. `None`, `()`, a unit struct, and `serde_json::Value::Null` now write MATLAB `struct([])` by default rather than dropping the field, so `isfield` reports true and MATLAB code can reference the field unconditionally. A field that is present also no longer needs `#[serde(default)]` to read back. Set `NullPolicy::Omit` for the old output.
