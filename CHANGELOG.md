...
- Reading persistent free-space managers (issue [#21](https://github.com/stephenberry/hdf5-pure/issues/21)). For a file written with `persist = true`, `File::persisted_free_space()` follows the File Space Info message's manager addresses to the on-disk free-space-manager blocks (the `FSHD` header and `FSSE` section list) and returns the tracked free `(address, length)` regions. The block parser is verified byte-for-byte against HDF5 1.14.6 and end-to-end against a file the reference C library writes with a deleted dataset.
- Writing persistent free space on disk, completing issue [#21](https://github.com/stephenberry/hdf5-pure/issues/21). `FileBuilder::with_file_space_strategy(strategy, /* persist */ true, threshold)` now records the persist intent instead of failing (a brand-new file has no free space, so it writes the persist flag with empty managers, exactly as the C library does). An `EditSession` opened on a file created with `persist = true` then seeds its free list from the on-disk managers on open and, on each commit, serializes the free list back to a freshly written `FSHD`/`FSSE` pair and an updated superblock-extension File Space Info message — so freed space survives close/reopen and is reused across sessions, by this crate and the reference C library alike. A persisting commit retains freed space (recording it on disk) rather than truncating it; the manager blocks are appended past all live data and the superblock is repointed last, so a crash before the repoint leaves the prior file intact. The serialized `FSHD`/`FSSE` bytes are byte-for-byte identical to the C library's (including the Jenkins checksums it verifies), and the round trip is validated against the C library: it reads back a persisted file this crate writes — recovering the strategy, the survivors, and (via `H5Fget_freespace`) the free space itself.

### Fixed

- The reference HDF5 C library could read hdf5-pure-created files but could not add objects to them: opening one read-write and creating a dataset or group failed with "message type not found". Every group's object header was written with a Link Info message (type `0x0002`) but no Group Info message (type `0x000A`), and the C library requires the pair — on the new-style-group path `H5G_obj_insert` reads the Group Info message unconditionally before inserting a link, so a group without it is readable but not writable by the C library. Every group header this crate writes (the root and subgroups, through `FileBuilder`, `EditSession`, and `repack`) now carries a minimal Group Info message (version 0, no optional fields, so the C library applies its defaults of max-compact 8 / min-dense 6), and an `EditSession` heals an older file's group headers that lack one whenever it rewrites them. Validated against the C library: it opens a file this crate wrote and adds datasets and subgroups to both the root and an hdf5-pure-created subgroup, including after an in-place `EditSession` edit, and hdf5-pure reads those C-added objects back. Read-only access was never affected, and the on-disk format is unchanged for readers.
