...
- `File::metadata_cache_stats` reports what a `MetadataCacheConfig` budget bought — hit rate, evictions, reads too large to admit, entries an in-place write invalidated, and current occupancy — so the budget can be measured rather than guessed; `File::reset_metadata_cache_stats` clears the counters without evicting. Together they are the `H5Fget_mdc_hit_rate` / `H5Fget_mdc_size` / `H5Freset_mdc_hit_rate_stats` analogue; the adaptive-resize policy of `H5AC_cache_config_t` is still not modeled ([#353](https://github.com/stephenberry/hdf5-pure/issues/353)).
