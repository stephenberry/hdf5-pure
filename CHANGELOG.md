...
- `FileAccessProperties::with_page_buffer_size` gives a read-write session a write-back page buffer, the `H5Pset_page_buffer_size` analogue: dirty pages live across the ordering barriers inside a commit or append, taking a session of 32 chunk appends from 160 writes to 10. Such a session marks the file in the superblock for its lifetime, so one that dies mid-flush leaves a file this crate and the C library both refuse rather than one that reads clean; it requires a paged version-3 file, a 1 MiB budget and `SyncPolicy::OnClose`, and it repays the mark's two `fsync`s only over a long session ([#308](https://github.com/stephenberry/hdf5-pure/issues/308)).
