...
- **Breaking:** An object reference supplied as an *address* — `with_reference_data`, `with_raw_data`, or an in-file `File::copy` re-emitting a source's references — is screened against the same commit's deletions, as one named by path already was; it was written as-is and left pointing into reclaimed space. Copying a *chunked* object-reference dataset, or an object with a shared (SOHM) attribute, is refused beside a delete rather than screened ([#317](https://github.com/stephenberry/hdf5-pure/issues/317)).
