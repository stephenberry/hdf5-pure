...
- `File::open_rw` opens a file for reading **and** in-place appends, and `Dataset::append` grows a chunked, unlimited, Extensible-Array-indexed dataset through an owned handle — the appended data reads back through the same handle — reusing the crash-atomic engine behind `AppendWriter` (unfiltered any-length, filtered whole-chunk); a write on a read-only file returns `Error::ReadOnly` ([#148](https://github.com/stephenberry/hdf5-pure/issues/148)).
