...
- An attribute too large for dense (fractal-heap) storage is now refused with the new `FormatError::DenseAttributeTooLarge` naming it, instead of being written into a heap that read back empty and aborted an assertion-enabled reference C library; more than 65,535 attributes on one object is refused with `FormatError::TooManyAttributes` ([#191](https://github.com/stephenberry/hdf5-pure/issues/191)).
- Dense attribute sets whose total passes 64 KiB are no longer refused by `EditSession` copies: the bound now tracks each attribute's size, which is what the emitter is actually limited by, so multi-megabyte sets of individually small attributes are written and copied normally ([#191](https://github.com/stephenberry/hdf5-pure/issues/191)).
