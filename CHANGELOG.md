...
- A `maxshape` whose chunk index would leave more than 4,194,304 elements unused is refused; the index is emitted in one pass, so those elements are written to the file too. Unused elements come from a `maxshape` wider than the shape in a dimension other than the one the dataset grows along ([#299](https://github.com/stephenberry/hdf5-pure/issues/299)).
