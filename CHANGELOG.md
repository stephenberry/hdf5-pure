...
- `MatError::from_source` carries a caller's own error type whole through the builder's closures and `DataProducer`, into the new `MatError::Source` variant, so an embedding crate recovers it with `Error::source` and `downcast_ref` instead of flattening it to text through `MatError::Custom`.
