...
- **Breaking:** An object reference supplied as an *address* — `with_reference_data`, `with_raw_data`, or an in-file `File::copy` re-emitting a source's references — is screened against what the same commit vacates, as one named by path already was; it was written as-is and left naming freed storage. A supplied address is also refused when it names a header the commit rewrites elsewhere, pointing at `with_path_references`, which resolves to where the object lands ([#317](https://github.com/stephenberry/hdf5-pure/issues/317)).
