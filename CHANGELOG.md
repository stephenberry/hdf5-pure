...
- **Breaking:** Extensible-Array and Fixed-Array chunk indexes have their checksums validated on read, as the reference C library does, so a torn or bit-rotted index is refused rather than decoded as sound. A file with an already-corrupt index that this crate used to read now reports `ChecksumMismatch` ([#312](https://github.com/stephenberry/hdf5-pure/issues/312)).
