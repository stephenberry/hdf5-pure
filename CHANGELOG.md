...
- Nested MATLAB **MCOS objects now decode**: a `string` / `datetime` / `duration` / `categorical` / struct / user-class value embedded inside another opaque object — most visibly a **`table`** or **`timetable`** column or its row-times — resolves to its real value instead of the raw `uint32` reference metadata, so a table reads back as its `data` columns plus `varnames` / `nrows` (a dedicated typed table view is still to come) ([#114](https://github.com/stephenberry/hdf5-pure/issues/114)).
